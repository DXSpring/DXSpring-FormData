...
// Object Service
export const ArrayService = ({ data, key, customKey }: IArrayService): any => {
    const FinalData: any = {};
    ...
    if (data && !!data.length) {
        for (let Index = 0; Index < data.length; Index++) {
            ...
            if (data[Index] instanceof Array && !!data[Index].length) {
                const ArrayData = ArrayProvider({ data: data[Index], key: `${key ? key : ''}${customKey ? '[' + customKey + ']' : ''}[${Index}]`, customKey: customKey ? customKey : 'array' });
